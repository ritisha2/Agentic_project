...
# categorized_dir is only needed when re-building the registry from CSVs.
# For runtime inference the pre-built registry JSON is used (registry_file=).
...
class WellDiagnosticEngine:
    ...
    def __init__(
        ...
        # Use the pre-built JSON registry by default (portable — no CSV dir needed).
        resolved_registry = registry_file or _DEFAULT_REGISTRY_FILE
        resolved_dir = categorized_dir or _DEFAULT_CATEGORIZED_DIR

        self.registry = WellCalibrationRegistry(
            categorized_dir=resolved_dir,
    ...
    def evaluate_live_telemetry(
        ...
        """
        Ingests a raw, un-normalized live site telemetry packet, normalizes it, and returns the diagnostic card.
        """
        # 1. Real-time normalization and dynamics computation
        norm_result = self.normalizer.normalize_live_telemetry(well_id, raw_telemetry, prev_telemetry)
