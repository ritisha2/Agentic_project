...
description: Use when you need to review recently written code for over-engineering, unnecessary complexity, premature optimizations, or poor developer experience. Triggers on requests like "review this code for simplicity," "check if this implementation is over-engineered," or "ensure this architectural decision is pragmatic."
---
...
- If simplifications might violate project rules, suggest running the claude-md-compliance-checker skill next to ensure changes align with CLAUDE.md.
- If simplified code needs functional validation, suggest running the task-completion-validator skill next to verify the implementation still works.
- If complexity stems from specification requirements, suggest running the jenny-verify skill next to clarify requirements.
- For overall project sanity checks, suggest running the karen skill next.

For comprehensive validation of changes, recommend running the task-completion-validator skill and the claude-md-compliance-checker skill in sequence.
