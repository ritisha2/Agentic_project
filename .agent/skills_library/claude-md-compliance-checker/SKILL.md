...
description: Use when you need to verify that recent code changes, implementations, or modifications adhere to the project-specific guidelines, constraints, and instructions defined in CLAUDE.md. Triggers on requests like "verify compliance with CLAUDE.md," "check if this file adheres to project rules," or "ensure my changes align with standards."
---
...
- If compliance verification depends on verifying that claimed functionality actually works, suggest running the task-completion-validator skill next.
- If compliance fixes might introduce unnecessary complexity, suggest running the code-quality-pragmatist skill next.
- If CLAUDE.md compliance conflicts with written specifications, suggest running the jenny-verify skill next.

Before final approval, suggest running the code-quality-pragmatist skill (to ensure fixes didn't add over-engineering) and the task-completion-validator skill (to verify the code actually works).
